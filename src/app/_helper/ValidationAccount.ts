export default class AccountValid {
  ...
  static BadResponse = (nameProp: string | string[]): AccountValid => {
    ...
    if (typeof nameProp === typeof "") inCorrects.push(nameProp.toString());
    else inCorrects = [...Array.from(nameProp)];
  ...
  static ValidationAccount = ({
    ...
    if (!/^[a-zA-Z]+[a-zA-Z_0-9]{2,}$/.test(username))
      return AccountValid.BadResponse("username");
    if (!/^[a-zA-Z_0-9]{3,}$/.test(password))
      return AccountValid.BadResponse("password");
    if (repassword && password !== repassword)
      return AccountValid.BadResponse("repassword");
    if (email && this.validateEmail(email))
      return AccountValid.BadResponse("email");
